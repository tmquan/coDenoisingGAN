...
class Model(GANModelDesc):
    ...
    def build_graph(self, image, label, noise):
        ...
        with tf.variable_scope('gen'):
            ...
            with tf.variable_scope('image'):
                with tf.variable_scope('label'):
                    l1_codes, _ = self.encoder(l0)
                with tf.variable_scope('noise'):
                    n1_codes, _ = self.encoder(n0)
                with tf.variable_scope('image'):
                    i1, _              = self.decoder(l1_codes)
                    

            with tf.variable_scope('label'):
                with tf.variable_scope('image'):
                    i1_codes, _ = self.encoder(i1)
                with tf.variable_scope('noise'):
                    n1, _              = self.decoder(i1_codes)
                    
                with tf.variable_scope('label'):
                    l1, _              = self.decoder(i1_codes)
                    

                # PPC branch
                with tf.variable_scope('image'):
                    i2_codes, _ = self.encoder(i0)
                with tf.variable_scope('noise'):
                    n2, _              = self.decoder(i2_codes)
                    
                with tf.variable_scope('label'):
                    l2, _              = self.decoder(i2_codes)
                    

            with tf.variable_scope('image'):
                with tf.variable_scope('label'):
                    l2_codes, _ = self.encoder(l2)
                with tf.variable_scope('noise'):
                    n2_codes, _ = self.encoder(n2)
                with tf.variable_scope('image'):
                    i2, _              = self.decoder(l2_codes)
...
if __name__ == '__main__':
    ...
    if args.sample:
        ...
        prefix="result/SeparatelyCyclicDenoise/"
